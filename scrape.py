...
def extract_work(data):
    ...
    for experience in data['experiences']['jobs']:
        ...
        if date is not 'N/A' and time_length >= 1 and 'summer' not in experience['title'].lower():
            if date < first_work:
                first_work = date
...
def scrape(scraper, url, headless=True):
    """
...
def scrape_profile(scraper, url):
    """
    ...
    international = pickle.load(open("int_schools.p", "rb"))

    data = scrape(scraper, url)                # scrapes the profile located at URL
...
def handler(event, context):
    ...
    scraper = ProfileScraper(driver=driver_type, email=args["email"], password=args["password"], driver_options=driver_options)

    fullName, firstName, lastName, location, locs, undergrad, all_schools, \
    yrs_experience, headline, int_hs, undergrad_yr = scrape_profile(scraper, args["url"])

    return {"statusCode": 200,
            "body": json.dumps({"fullName": fullName,
                                "firstName": firstName,
                                "lastName": lastName,
                                "location": location,
                                "otherLocs": locs,
